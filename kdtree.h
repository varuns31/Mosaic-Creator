...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* buildtree(vector<Point<Dim>>& newPoints,unsigned left, unsigned right,int currdim);
    Point<Dim>& select(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned k, int dim);
    unsigned partition(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned pivotIndex, int dim);
    
    void deletehelper(KDTreeNode * myRoot);
    KDTreeNode* copyhelper(KDTreeNode* OtherRoot);
